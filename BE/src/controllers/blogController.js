...
// @route   GET /api/blogs?shop=...&category=...
// @access  Public
const getPublicBlogs = async (req, res) => {
    try {
        const { shop, category, search, page = 1, limit = 9 } = req.query;
        ...
        const [blogs, total] = await Promise.all([
            Blog.find(query).populate('shop', 'name slug logo').populate('author', 'fullName')
                .select('-content').sort({ isPinned: -1, publishedAt: -1, createdAt: -1 }).skip(skip).limit(Number(limit)),
            Blog.countDocuments(query)
        ]);
